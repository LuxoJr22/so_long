...
/*   Updated: 2023/02/08 21:32:47 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void reset_tears(t_game *game, int force)
{
	...
	while(next)
	{
		if (next->is_active == 0 || force)
		{
			...
			next->pos.y -= 30;
			if (!force)
				create_lanims(game, get_anim(game, "sprite/tears/splash/tile", 14, 0), next->pos, 1);
			free(next);
			next = first->next;
		}
		else
...
void	move_tears(t_game *game)
{
	...
	while (tears)
	{
		...
		if (tears->pos.x > 900 || tears->pos.x < 32 || tears->pos.y > 500 || tears->pos.y < 32)
			tears->is_active = 0;
        while (i < game->nb_enemies && tears->is_active)
        {
            if (tears->pos.x / 64 == game->enemies[i]->px && tears->pos.y / 64 == game->enemies[i]->py)
            {
